...
var url_server="http://q1531y6702.51mypc.cn:15155/libraryServer";
angular.module('wechat.controllers', ['ionic'])
...
.controller('tabCtrl', function($scope, $state,$http) {
	...
	init();
	function init(){		
		$scope.g_tab={'g_lendExBooks':0};
    	showLendExBook();
	}
	
        //显示逾期图书角标
    function showLendExBook(){
    	var userCode=window.localStorage.getItem("userCode");
    	if(userCode!=null){
    		var url=url_server+"/mylib/getLendExCount/?userCode="+userCode;
    		var data={};
	    	$http.get(url,{params:data}).success(function(result) {
			   $scope.g_tab.g_lendExBooks = result.lendExCount;
			}).error(function(error){
    ...
    $scope.tabToggle=function(state1){
    	showLendExBook();
    	$state.go(state1);
    }
...
.controller('searchCtrl', function($scope, $state,$ionicPopup) {
	//***图书检索***
	init();
    ...
    function init(){
    	$scope.searchObj={keyword:'',kind:'bookName'};
    }
    
    $scope.search=function(){
    	var keyword=$scope.searchObj.keyword;
    	var kind=$scope.searchObj.kind;
    	if(keyword==""){
    		$scope.showAlert("提示","请输入关键字");
    		return;
    	}
...
.controller('searchResultCtrl', function($scope, $state,$stateParams,$http,$ionicLoading) {
    ...
    function init(){
    	...
    	var url=url_server+"/search/search/?keyWord="+keyword+"&kind="+kind;
	...
	$scope.loadMore=function(){
		...
		var keyword=$stateParams.keyword;
    	var kind=$stateParams.kind;
    	var url=url_server+"/search/search/?keyWord="+keyword+"&kind="+kind+"&thePage="+thePage;
...
.controller('mylibInfoCtrl', function($scope, $state,$http) {
    ...
    function init(){
    	...
    	$http.get(url,{params:data}).success(function(result) {
    		if(result.state=="F"){
    			...
    			$scope.kind="barcode";
    			$scope.kindName="二维码";
    			var url=url_server+"/qr.jsp?content="+result.userInfo.userCode+"&kind=barcode";
    	        $scope.qrPic=url;
    		}		   
		}) 
    }
    ...
    $scope.toggleIt=function(){
    	if($scope.kind=="barcode"){
    		$scope.kind="qr";
    		$scope.kindName="条形码";
    		var url=url_server+"/qr.jsp?content="+$scope.userInfo.userCode+"&kind=qr";
    	    $scope.qrPic=url;
    	}else{
    		$scope.kind="barcode";
    		$scope.kindName="二维码";
    		var url=url_server+"/qr.jsp?content="+$scope.userInfo.userCode+"&kind=barcode";
    	    $scope.qrPic=url;
    	}
    }
})
...
.controller('loginCtrl', function($scope, $state,$http,$stateParams) {
    ...
    $scope.login=function(){    	
    	var url=url_server+"/login/checkPwdAll/?userCode="+$scope.userInfo.userCode+"&userPwd="+$scope.userInfo.userPwd;
		...
		$http.get(url,{params:data}).success(function(result,status,headers,config){
			if(result.state=="F"){
				...
				//设置
				window.localStorage.setItem("userCode",result.userCodeXX);
    ...
    function init(){
    	...
    	//去允许的验证方式
    	$scope.checkKind="借书证号"
    	var url=url_server+"/login/getLoginKind";
    	var data={'foobar': new Date().getTime()+Math.random()};
    	$http.get(url,{params:data}).success(function(result,status,headers,config){
    		if(result.checkPhone=="T"){
    			$scope.checkKind=$scope.checkKind+"/手机号";
    		}
    		if(result.checkId=="T"){
    			$scope.checkKind=$scope.checkKind+"/身份证";
    		}
    		if(result.checkEmail=="T"){
    			$scope.checkKind=$scope.checkKind+"/邮箱";
    		}
    	});
    }
